...
interface ClientWorkCardProps extends ClientJob {
    vehicle: string;
    progress?: { notes: string; hoursWorked: number } | null; 
    onApproveService?: (
        workId: number,
        approveType: string,
        comment: string
            ...
            <div className="flex-1 min-w-0">
                ...
                {progress ? (
                    <p className="text-sm text-gray-700 mt-1">
                        Progreso: {progress.notes} ({progress.hoursWorked}h)
                    </p>
                ) : (
                    <p className="text-sm text-gray-400 mt-1">Sin progreso</p>
                )}
                {showAlert &&
                    Alert({
                        type: "error",
                    ...
                    {InvoiceRequest?.invoices?.length > 0 && (
                        <section className="bg-gray-50 rounded-lg p-3">
                            ...
                            <ul className="space-y-2">
                                {InvoiceRequest.invoices.map((item: any, index: Key | null | undefined) => (
                                    <li
                                        key={index}
